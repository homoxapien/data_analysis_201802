...
```{r}
social <- read.csv("data/2015_SPI.csv")
str(social)
```
...
```
So we proceed with k=2, i.e. divide countries into 2 clusters.
```{r}
set.seed(2018)
pamK2 <- pam(soc.happy[, 2:10], diss=FALSE, 2, keep.data=TRUE)
# Number of countries assigned in the three clusters
fviz_silhouette(pamK2)

## which countries were typical of each cluster
soc.happy$Country[pamK2$id.med]
```
Plot them on the PC1-PC2 plane
```{r}
soc.happy['cluster'] <- as.factor(pamK2$clustering)
